######################################################################
## Target Species Condition indicators - for SC18 paper 2022
######################################################################
...
## Indicator 1: Mean length of target species
## Indicator 2: Mean condition factor (K_rel) from LL catch

## Key changes from 2021: 
## 1) Addition of new data from 1990 to 2021 inclusive.
## 2) Begin SKJ K_rel time series at 1998, and then continue from 2010 to 2020, as data are few in other years.
...
setwd("P:/OFPEMA/WCPFC/SC18/Ecosystem Indicators/Fish condition")
...
bio_data<-bio_data[bio_data$yy%in%1990:2020,] # subset for 1990 to 2020 inclusive
...
obsv_data<-obsv_data[obsv_data$yy%in%1990:2020,] # subset for 1990 to 2020 inclusive
...
port1_LW<-port1_LW[port1_LW$yy%in%1990:2020,] # subset for 1990 to 2020 inclusive
...
port2_LW<-port2_LW[port2_LW$yy%in%1990:2020,] # subset for 1990 to 2020 inclusive
...
# Plot Indicators 1 and 2 by species
par(mfrow=c(1,1))
plot(L.means_BET$length_cm~L.means_BET$yy, type="l", col="red", lwd=2, 
     ...
     xaxt="n")
axis(side=1, at=seq(1990,2020,2), labels=(as.character(seq(1990, 2020, by=2))), las=2)
lines(L.means_YFT$length_cm~L.means_YFT$yy, type="l", col="goldenrod1", lwd=2)
...
PS.bio_SKJ<-bio_SKJ[bio_SKJ$gear_id=="S" & bio_SKJ$yy%in%1990:2020,] # get PS records only
PS.bio_SKJ_LW<-bio_SKJ_LW[bio_SKJ_LW$gear_id=="S" & bio_SKJ_LW$yy%in%1990:2020,]
LL.bio_SKJ<- bio_SKJ[bio_SKJ$gear_id=="L" & bio_SKJ$yy%in%1990:2020,] # get LL records only
LL.bio_SKJ_LW<-bio_SKJ_LW[bio_SKJ_LW$gear_id=="L" & bio_SKJ_LW$yy%in%1990:2020,] # no records
PL.bio_SKJ<-bio_SKJ[bio_SKJ$gear_id=="P" & bio_SKJ$yy%in%1990:2020,] # get PL records only
PL.bio_SKJ_LW<-bio_SKJ_LW[bio_SKJ_LW$gear_id=="P" & bio_SKJ_LW$yy%in%1990:2020,] # only 37 records
...
PS.obsv_SKJ<-PS.obsv_SKJ[PS.obsv_SKJ$yy%in%1990:2020,] # subset for 1990 to 2020 inclusive
...
LL.obsv_SKJ<-LL.obsv_SKJ[LL.obsv_SKJ$yy%in%1990:2020,] # subset for 1990 to 2020 inclusive
...
port_SKJ<-port_SKJ[port_SKJ$yy%in%1990:2020,] # subset for 1990 to 2020 inclusive
...
# For PL only two years of data were available (2009, 2019) in 'PL.bio_SKJ_LW',
# so not useful for this analysis.
# Present K_rel for LL only for SC18 paper.
...
plot(LL.K_rel.means_SKJ$K_rel~LL.K_rel.means_SKJ$yy, type="o", col="dodgerblue", lwd=2, las=2)
abline(a=1, b=0, lty=2)

# Re-check numbers of records by year
table(LL.SKJ_LW$yy) # very low numbers in 1997, 2007, 2008, 2009, so use only present records from 1998, and 2010:2020.
LL.K_rel.means_SKJ<-LL.K_rel.means_SKJ[LL.K_rel.means_SKJ$yy%in%c(1998, 2010:2020),]
...
plot(LL.K_rel.means_SKJ$K_rel~LL.K_rel.means_SKJ$yy, type="p", col="dodgerblue", lwd=2, 
     ylim=c(0.80,1.30), las=2, xlab="", ylab=expression(paste("Mean SKJ ", italic("K")[rel], " from LL catch")), 
     xaxt="n")
lines(LL.K_rel.means_SKJ$K_rel[2:12]~LL.K_rel.means_SKJ$yy[2:12], col="dodgerblue", lwd=2)
axis(side=1, at=seq(2000,2018,2), labels=(as.character(seq(2000, 2018, by=2))), las=2)
abline(a=1, b=0, lty=2)

# Save to .csv's
write.csv(PS.means_SKJ, "PS_L.means_SKJ.csv", row.names=F)
write.csv(LL.means_SKJ, "LL_L.means_SKJ.csv", row.names=F)
write.csv(PL.means_SKJ, "PL_L.means_SKJ.csv", row.names=F)
