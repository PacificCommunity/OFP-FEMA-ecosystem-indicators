...
bind_rows(tmp1,tmp2,tmp3) |>
  ...
  xlim(1990,2023)
p
ggsave(p, file = paste0(results_wd, 'long_deviance/figs/modeled_vs_COG_indices_enso_modL.png'))
...
combined <- (p1 + p2 + p3 + p4 + p5) + plot_layout(ncol = 2)

p <- 
ggdraw() +
  draw_plot(combined) +
  draw_label("Predicted Longitude", x = 0.02, y = 0.5, angle = 90, vjust = 1, size = 12, fontface = 'bold')
ggsave(p, file = paste0(results_wd, 'long_deviance/figs/modL_var_response_plot.png'), 
       width = 7, height = 10, units = 'in', dpi = 100)
...
terms <- c("s(yy,flag)", "s(mm)", "s(latd)", "set_type", "oniF")
